 ...
 */
export async function queryProjectContext(session, limit = 3) {
    const bin = binParts();
    if (!bin || !session.projectId)
        return undefined;
    try {
 ...
 */
export async function ingestCleanedLog(session, cleanedPath) {
    const bin = binParts();
    if (!bin)
        return;
    const tags = ["voicelog", ...(session.projectId ? [session.projectId] : [])].join(",");
    try {
        await execFileAsync(bin.cmd, [...bin.base, "ingest", cleanedPath, "--tags", tags, "--source", "voicelogger"], { timeout: 30000, maxBuffer: 1024 * 1024 });
    }
    catch {
