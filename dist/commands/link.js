...
export async function linkCommand(args) {
    ...
    if (!id || !projectId) {
        console.error("usage: voicelogger link <session|latest> <projectId> [--touch] [--reason <r>] [--note <n>] [--no-ledger]");
        process.exit(1);
    }
    ...
    if (!config.ledgerEnabled || args.includes("--no-ledger"))
        return;
    const noteText = optValue(args, "--note") ??
    ...
    if (noteRes.ok) {
        ...
        console.warn(`! ledger note failed (${exitLabel(noteRes.code)}): ${noteRes.stderr.trim() || "check ledger-cli auth"} — local link still saved`);
    }
    if (args.includes("--touch")) {
        ...
        if (touchRes.ok) {
            ...
            console.warn(`! ledger touch failed (${exitLabel(touchRes.code)}): ${touchRes.stderr.trim() || "check ledger-cli auth"}`);
        }
    }
}
