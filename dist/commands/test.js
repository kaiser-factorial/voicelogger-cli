...
export async function testCommand(args) {
    ...
    if (recipe.kind === "unknown") {
        console.error(`✗ couldn't detect a project type at ${absPath} (looked for tauri.conf.json, go.mod, and ` +
    ...
    if (result.status === "running-no-url") {
        console.log("  server is running but no URL was detected in its output — open it manually.\n" +
...
async function runProd(absPath, recipe, args) {
...
async function runGoCli(absPath, recipe) {
