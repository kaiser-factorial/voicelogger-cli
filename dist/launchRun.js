...
function trialCandidate(cmd, cwd) {
    return new Promise((resolve) => {
        ...
        const onData = (buf) => {
            ...
            if (match)
                finish({ ok: true, url: match[0], output });
        };
        child.stdout?.on("data", onData);
        child.stderr?.on("data", onData);
        child.on("exit", (code) => finish({ ok: false, exitCode: code, dud: !pastGrace, output }));
        child.on("error", (err) => finish({ ok: false, exitCode: null, dud: !pastGrace, output: cap(output + `\n[spawn error] ${err.message}`) }));
...
async function trialNodeCandidates(candidates, cwd) {
    let last;
    for (const script of candidates) {
        ...
        last = { cmd, outcome };
        if (outcome.ok)
            return { cmd, outcome }; // success (with or without a scraped URL)
        if (!outcome.dud)
            return { failed: outcome, cmd }; // exited past grace, no URL — a real
...
async function spawnAndPollKnownUrl(cmd, cwd, url) {
    let exitCode;
    let output = "";
    const child = spawnDetached(cmd, cwd);
    const onData = (buf) => {
 ...
 */
export async function resolveDevServer(recipe, opts = {}) {
    const dev = recipe.dev;
    if (!dev)
        throw new Error(`resolveDevServer: recipe has no dev entry (kind=${recipe.kind})`);
...
//# sourceMappingURL=launchRun.js.map