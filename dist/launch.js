...
export async function loadLaunchCache() {
    try {
        return JSON.parse(await readFile(launchFilePath(), "utf8"));
    }
    catch {
        return {};
    }
}
export async function saveLaunchRecipe(absPath, recipe) {
...
export const NODE_DEV_CANDIDATES = ["dev", "serve", "start"];
...
async function findTauriConf(absPath) {
 ...
 */
async function guessGoBinaryName(absPath) {
 ...
 */
export async function detectProject(absPath) {
    ...
    const pkg = await readJson(path.join(absPath, "package.json"));
    if (pkg) {
