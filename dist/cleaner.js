...
async function cleanWithAnthropic(rawBody, inputs) {
...
async function cleanWithOpenAICompat(rawBody, inputs) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        process.stderr.write(`  rate-limited — retrying in ${delaySec}s (${attempt + 1}/${MAX_RETRIES})…\n`);
        await new Promise((r) => setTimeout(r, delaySec * 1000));
    }
    if (!res.ok) {
        const errBody = await res.text().catch(() => res.statusText);
        if (res.status === 429) {
            throw new Error(`${config.anthropicModel} is rate-limited — try a different model:\n` +
