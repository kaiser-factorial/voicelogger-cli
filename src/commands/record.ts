...
import type { TestLogServerHandle } from "../testLogServer.js";
import { startTestLogServer } from "../testLogServer.js";
import { readRawBody, writeSession } from "../store.js";
import type { TestLogScope, VoiceLogSession } from "../types.js";
...
export async function recordCommand(args: string[]): Promise<void> {
  ...
  const scope: TestLogScope | undefined = testLog
  ...
  if (testLog) console.log(`  test-log: narrator ${speaker}, scope ${scope}`);

  let server: TestLogServerHandle | undefined;
  let stopped = false;
  const finish = async () => {
    if (stopped) return;
    stopped = true;
