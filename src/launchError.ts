 ...
 */
export function hasLaunchSummarizerAuth(): boolean {
  return Boolean(process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_AUTH_TOKEN);
}

async function summarize(output: string): Promise<string | undefined> {
  if (!hasLaunchSummarizerAuth()) return undefined;
  try {
    ...
    const response = await client.messages.create({
      ...
      max_tokens: 300,
      system:
        "You summarize a failed dev-server/build launch for a developer. Given raw captured " +
 ...
 */
export async function buildHandoffMessage(failure: LaunchFailure): Promise<string> {
